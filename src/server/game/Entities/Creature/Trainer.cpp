...
namespace Trainer
{
    ...
    void Trainer::SendSpells(Creature const* npc, Player const* player, LocaleConstant locale) const
    {
        ...
        bool canLearnPrimaryProfession = player->GetFreePrimaryProfessionPoints() > 0;
        ...
        for (Spell const& trainerSpell : _spells)
        {
            ...
            trainerListSpell.ProfessionButton = primaryProfessionFirstRank ? canLearnPrimaryProfession : false;
    ...
    void Trainer::TeachSpell(Creature const* npc, Player* player, uint32 spellId) const
    {
        ...
        if (player->GetMoney() < uint64(moneyCost))
        {
